...
// Controller function to handle the webhook for deposit transactions.
// When a new deposit transaction is posted in the user's bank account in the core banking system,
// We receive a notification to aggregate this data into our database.
    const handleTransactionWebhookDeposits = async (req, res) => {
        const {
            transaction_id,
            transaction_date,
            detail,
        ...
        if(!transaction_id || !transaction_date || !detail || !transaction_amount || !db_cr || !account_id){
        ...
        try {
            // Checking if account_id exists in the database, as we only store data from users
            ...
            (transaction_id, transaction_date, detail, amount, db_cr, account_id)
            VALUES ($1, $2, $3, $4, $5, $6)
        `;

            const values = [
                transaction_id, transaction_date, detail, transaction_amount, uppercaseDbCr, account_id
            ];
    ...
    module.exports = {
        handleTransactionWebhookDeposits
    };
